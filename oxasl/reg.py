...
def run(wsp, redo=False, struc_flirt=True, struc_bbr=False, use_basil_wsp=None):
    if wsp.reg is None:
        wsp.sub("reg")
        redo = True
    ...
    if redo:
        ...
        get_ref_imgs(wsp, use_basil_wsp)
        reg_asl2calib(wsp)
        reg_asl2struc(wsp, flirt=struc_flirt, bbr=struc_bbr)
        reg_asl2custom(wsp)

def get_ref_imgs(wsp, use_basil_wsp=None):
    """
    ...
    if use_basil_wsp:
        ...
        pwi = np.copy(use_basil_wsp.finalstep.mean_ftiss.data)
        pwi[pwi < 0] = 0
        wsp.reg.aslref = Image(pwi, header=wsp.basil.finalstep.mean_ftiss.header)
